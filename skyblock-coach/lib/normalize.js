...
import {
  xpToSkillLevel, xpToCatacombsLevel, xpToHotmTier, SKILL_CAPS,
...
export async function normalize(profilesResponse, uuid, username) {
  ...
  const lvl = (name) => xpToSkillLevel(skillXp(name), SKILL_CAPS[name] || 60);
  const skills = {
    combat: lvl("combat"), farming: lvl("farming"), mining: lvl("mining"),
    foraging: lvl("foraging"), fishing: lvl("fishing"), enchanting: lvl("enchanting"),
    alchemy: lvl("alchemy"), taming: lvl("taming"),
  };
  ...
  try {
    ...
    const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error("networth timeout")), 28000));
